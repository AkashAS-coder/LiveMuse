...
class MusicRoom:
    ...
    def add_chat_reaction(self, message_id, username, emoji):
        print(f"Looking for message {message_id} in {len(self.chat_messages)} messages")
        for i, message in enumerate(self.chat_messages):
            print(f"Message {i}: id={message['id']}, looking for {message_id}")
...
def handle_chat_reaction(data):
    ...
    username = data['username']
    emoji = data['emoji']
    print(f"Received reaction: {emoji} from {username} for message {message_id} in room {room_id}")
    if room_id in rooms:
        room = rooms[room_id]
        updated_message = room.add_chat_reaction(message_id, username, emoji)
        if updated_message:
            print(f"Reaction added successfully. Updated reactions: {updated_message['reactions']}")
            emit('chat_reaction', {
                'message_id': message_id,
                'reactions': updated_message['reactions']
            }, room=room_id)
        else:
            print(f"Failed to add reaction - message {message_id} not found")
    else:
